...
struct DeviceCommandMessage {
  ...
  var command: DeviceCommandMessage.Command {
    get {return _storage._command ?? .lock}
  ...
  /// delete (staeful) or add signature (stateless)
  var reservation: DeviceReservationMessage {
    get {return _storage._reservation ?? DeviceReservationMessage()}
    set {_uniqueStorage()._reservation = newValue}
  }
  /// Returns true if `reservation` has been explicitly set.
  var hasReservation: Bool {return _storage._reservation != nil}
  /// Clears the value of `reservation`. Subsequent reads from it will return its default value.
  mutating func clearReservation() {_uniqueStorage()._reservation = nil}
...
extension DeviceCommandMessage: SwiftProtobuf.Message, SwiftProtobuf._MessageImplementationBase, SwiftProtobuf._ProtoNameProviding {
  ...
  static let _protobuf_nameMap: SwiftProtobuf._NameMap = [
    1: .same(proto: "command"),
    2: .same(proto: "reservation"),
  ]

  fileprivate class _StorageClass {
    var _command: DeviceCommandMessage.Command? = nil
    var _reservation: DeviceReservationMessage? = nil
  ...
  public var isInitialized: Bool {
    return withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      ...
      if _storage._reservation == nil {return false}
      if let v = _storage._reservation, !v.isInitialized {return false}
      return true
    }
  }
  ...
  mutating func decodeMessage<D: SwiftProtobuf.Decoder>(decoder: inout D) throws {
    ...
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      while let fieldNumber = try decoder.nextFieldNumber() {
        switch fieldNumber {
        case 1: try decoder.decodeSingularEnumField(value: &_storage._command)
        case 2: try decoder.decodeSingularMessageField(value: &_storage._reservation)
        default: break
        }
      }
    }
  ...
  func traverse<V: SwiftProtobuf.Visitor>(visitor: inout V) throws {
    try withExtendedLifetime(_storage) { (_storage: _StorageClass) in
      ...
      if let v = _storage._reservation {
